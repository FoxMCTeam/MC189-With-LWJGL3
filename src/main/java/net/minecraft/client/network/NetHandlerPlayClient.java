    ...
    public void handleResourcePack(SPacketResourcePackSend packetIn)
    {
        ...
        if (this.validateResourcePackUrl(s))
        {
            if (s.startsWith("level://"))
            {
                try
                {
                    ...
                    if (file2.isFile())
                    {
                        ...
                        Futures.addCallback(this.client.getResourcePackRepository().setServerResourcePack(file2), this.createDownloadCallback(), MoreExecutors.directExecutor());
                        return;
                    }
                }
            ...
            else
            {
                ...
                {
                    this.netManager.sendPacket(new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.ACCEPTED));
                    Futures.addCallback(this.client.getResourcePackRepository().downloadResourcePack(s, s1), this.createDownloadCallback(), MoreExecutors.directExecutor());
                }
                            ...
                            NetHandlerPlayClient.this.client.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback()
                            {
                                public void confirmClicked(boolean result, int id)
                                {
                                    ...
                                    if (result)
                                    {
                                        ...
                                        Futures.addCallback(NetHandlerPlayClient.this.client.getResourcePackRepository().downloadResourcePack(s, s1), NetHandlerPlayClient.this.createDownloadCallback(), MoreExecutors.directExecutor());
                                    }
                                    else
                                    {
                                        if (serverdata1 != null)
